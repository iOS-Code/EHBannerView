...
@interface NSTimer (YCTimer)

+ (NSTimer *)yc_timerWithTimeInterval:(NSTimeInterval)interval repeats:(BOOL)repeats block:(void (^)(NSTimer *))block;

@end

@implementation NSTimer (YCTimer)
...
- (void)setImageArray:(NSArray *)imageArray
{
    ...
    for (int i = 0; i < imageArray.count; i++) {
        if ([imageArray[i] isKindOfClass:[UIImage class]]) {
            ...
            else [_images addObject:[UIImage imageNamed:@"YCPlaceholder"]];
...
- (void)startTimer
{
    ...
    self.timer = [NSTimer yc_timerWithTimeInterval:_time < 1? DEFAULTTIME: _time repeats:YES block:^(NSTimer * _Nonnull timer) {
        [weakSelf nextPage];
    }];
...
- (void)downloadImages:(int)index
{
    ...
    NSBlockOperation *download = [NSBlockOperation blockOperationWithBlock:^{
        ...
        if (image) {
            ...
            if (weakself.currIndex == index) [weakself.currImageView performSelectorOnMainThread:@selector(setImage:) withObject:image waitUntilDone:NO];
            if (weakself.autoCache) [data writeToFile:path atomically:YES];
        }
    }];
